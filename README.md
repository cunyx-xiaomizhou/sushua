...
系统仅在项目根目录存在 `.git` 文件夹时启用后台版本检测。管理员每次进入管理后台时会请求一次远程 `version.json`?并展示当前版本、远程版本及版本特性。

版本检测只使用 PHP 文件读取和 cURL 请求，不调用 `exec`?`shell_exec`?`system`?`passthru`?`proc_open` 或其他命令执行函数。如果 cURL 扩展未安装或远程服务不可用，只会在版本页显示检测不可用，不会影响系统其他功能。

检测到新版本后，请通过服务器既有的 Git 部署流程拉取代码。更新前请先备份数据库、配置文件和上传文件，并在更新后检查数据库迁移和站点配置。
